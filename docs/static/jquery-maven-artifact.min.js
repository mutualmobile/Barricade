...
(function ($) {
    function downloadUrl(groupId, artifactId, version, type) {
        ...
        return "https://repo1.maven.org/maven2/" + groupPath + "/" + artifactId + "/" + version + "/" + artifactId + "-" + version + type
    }
    ...
    $.fn.artifactVersion = function (groupId, artifactId, callback) {
        ...
        var url = 'https://search.maven.org/solrsearch/select/?q=g:"' + groupId + '"+AND+a:"' + artifactId + '"&wt=json&json.wrf=?';
    ...
    $.fn.artifactVersions = function (groupId, artifactId, callback) {
        ...
        var url = 'https://search.maven.org/solrsearch/select/?q=g:"' + groupId + '"+AND+a:"' + artifactId + '"&wt=json&rows=10&core=gav&json.wrf=?';
        $.getJSON(url, function (response) {
            var versions = response.response.docs;
            if (versions.length == 0) {
                return
            ...
            for (var i = 0; i < versions.length; i++) {
                ...
                newVersions.push({name: version, url: downloadUrl(groupId, artifactId, version, ".jar")})
            }
